...
import re
import numpy as np
# from mimic_utils import get_label
...
sys.path.append('..')
# from visualizations.draw_single import find_report
# path = '../data/cmb_bbox_di_coord_loc.hdf5'
# f = h5py.File(path, 'a')
#
# f.close()
#
        ...
        for word in text_list:
            ...
            if len(d_loc[d_loc['location'] == word].values) != 0:
                if dict_attributes[anchor]['location'] == None:
                    ...
                    if word not in dict_attributes[anchor]['location']: # todo: when considering different identities with the same name, this may be a problem
                ...
                if dict_attributes[anchor]['type'] == None:
                    ...
                    if word not in dict_attributes[anchor]['type']:  # todo: when considering different identities with the same name, this may be a problem
                ...
                if dict_attributes[anchor]['level'] == None:
                    ...
                    if word not in dict_attributes[anchor]['level']:  # todo: when considering different identities with the same name, this may be a problem
...
def initial_library():
    ...
    data_path = f'data/lib'
    path = os.path.join(data_path, 'disease_lib.csv')
    d_d = pd.read_csv(path)

    path_lev = os.path.join(data_path, 'level_lib.csv')
    d_lev = pd.read_csv(path_lev)
    path_loc = os.path.join(data_path, 'location_lib.csv')
    d_loc = pd.read_csv(path_loc)
    path_t = os.path.join(data_path, 'type_lib.csv')
    d_t = pd.read_csv(path_t)
    path_ploc = os.path.join(data_path, 'postlocation_lib.csv')
    d_ploc = pd.read_csv(path_ploc)
    path_change = os.path.join(data_path, 'position_change.csv')
...
def gen_disease_json(print_test = False, stop=False, save=True, uniform_name=True, fixed_order=True):
    initial_library()
    nlp = None
    ...
    if save:
        disease_path = 'data/covid_labels.json'
...
def question_gen(less_yes_no=False, small_sample=False):
    print('question gen')
    initial_library()
    ...
    if less_yes_no:
        name = '../data/question_gen/mimic_pair_questions_lessYesNo.csv'
    else:
        name = '../data/question_gen/mimic_pair_questions.csv'
    if small_sample:
...
def save_h5(questions, answers, pos, label_start_idx, label_end_idx, feature_idx,max_seq = 60, times=0, length = 100):
    filename = os.path.join('../data/question_gen/datasets/VQA_mimic_dataset.h5')
    if times == 0:
...
def save_coco_format():
    path_splits = '../data/question_gen/datasets/splits_mimic_VQA.json'
    ...
    for name in ['train','val','test']:
        ...
        json.dump(dict, open('../data/question_gen/mimic_gt_captions_%s.json'%name, 'w'))
        image_list = []
        anno_list = []
        print('saved')
...
if __name__=='__main__':
    ...
    # find_keywords_in_report(keyword, background_words=background_word)
    ...
    ### EXECUTING PART ####
    gen_disease_json(print_test=False, stop= False, save=True, uniform_name=True, fixed_order=True)
    # question_gen(less_yes_no=False, small_sample=True) # generate question csv
    # if less_yes_no: keep only presence questions for yes/no
    # statistic()

    print('finished question gen. start generate dataset...')
    # transform_questions2dataset(simple = False) # transform csv dataset into h5py dataset, which is the input of the model
